...
namespace UnityEngine.UI.Extensions
{
    public class ScrollSnapBase : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IScrollSnap, IPointerClickHandler
    {
        ...
        {
            if (PrevButton) PrevButton.SetActive(false);
            if (NextButton) NextButton.SetActive(false);
        }

        [Tooltip("If true pages will loop")]
        public bool loopMode;
        public bool buttonsNotVisible;
        public bool buttonsAlwaysVisible;

        [Tooltip("If true buttons will be set active false otherwise will be interactable false")]
        ...
        public void PreviousScreen()
        {
            ...
            else
            {
                if (loopMode)
                {
                    ChangePage(_screensContainer.transform.childCount - 1);
                }
            }
        }
        ...
        private void ToggleNavigationButtons(int targetScreen)
        {
            ...
            if (buttonsNotVisible)
            {
                PrevButton.SetActive(false);
                NextButton.SetActive(false);
                return;
            }
            else if (buttonsAlwaysVisible)
            {
                if (_screensContainer.transform.childCount > 1)
                {
            ...
            if (!_isInfinite)
            {
                ...
                if (NextButton)
                {
                    if (HideButtons) { NextButton.SetActive(targetScreen < _screensContainer.transform.childCount - 1); }
                    else
                    {
